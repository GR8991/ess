# BESS LCOS Analysis - FINAL WORKING VERSION (Fixed Duplicate ID Issue)
# Version 3.1 - Full User Control with SOH, Augmentation, and Report Generation
...
if 'overbuild_data' not in st.session_state:
    st.session_state.overbuild_data = None
...
.section-header { background: linear-gradient(90deg, #003366, #0055aa); color: white; padding: 15px; 
                  border-radius: 5px; margin: 20px 0 10px 0; font-weight: bold; }
.info-box { background-color: #e3f2fd; padding: 12px; border-left: 4px solid #1976d2; 
...
def calculate_overbuild_cashflow(power_mw, energy_mwh, project_life, cycles, battery_cost, decline_rate,
    ...
    for year in years:
        ...
        if year == 0:
            cf = {"year": year, "soh": soh * 100, "capacity": available_capacity,
                  "capacity_revenue": 0, "energy_revenue": 0, "total_revenue": 0,
                  "fixed_opex": 0, "variable_opex": 0, "total_opex": 0,
            ...
            cf = {"year": year, "soh": soh * 100, "capacity": available_capacity,
                  ...
                  "capex": 0, "net_cf": total_rev - total_opex}
    ...
    summary = {
        ...
        "total_revenue_nominal": df[df["year"] > 0]["total_revenue"].sum(),
        "total_opex_nominal": df[df["year"] > 0]["total_opex"].sum(),
        "npv": npv,
...
def show_analysis_results(title, data, approach_type):
    """Display results with unique keys for each approach"""
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["💰 Cash Flow", "📊 Charts", "🔋 Capacity", "📋 Table"])
    ...
    with tab2:
        ...
        with col1:
            ...
            st.plotly_chart(fig, use_container_width=True, key=f"rev_{approach_type}")
        
        with col2:
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df["year"], y=df["soh"], mode="lines+markers",
                                    line=dict(color="purple", width=3), name="SOH"))
            fig.update_layout(title="Battery Degradation", xaxis_title="Year", yaxis_title="SOH (%)", height=400)
            st.plotly_chart(fig, use_container_width=True, key=f"soh_{approach_type}")
    
    with tab3:
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df["year"], y=df["capacity"], mode="lines+markers",
                                line=dict(color="orange", width=3), fill="tozeroy", name="Capacity"))
        fig.update_layout(title="Available Capacity", xaxis_title="Year", yaxis_title="MWh", height=500)
        st.plotly_chart(fig, use_container_width=True, key=f"cap_{approach_type}")
    ...
    with tab4:
        ...
        st.download_button(
            ...
            mime="text/csv",
            key=f"dl_{approach_type}"
        )

def show_comparison_analysis(overbuild_data, staged_data):
    """Comparison analysis with unique keys"""
    ...
    with col1:
        st.metric("NPV Advantage", f"${npv_adv:.2f}M", f"{(npv_adv / abs(overbuild_data['summary']['npv']) * 100):.1f}%")
    with col2:
        st.metric("CAPEX Savings", f"${capex_adv:.2f}M", f"{(capex_adv / overbuild_data['summary']['initial_capex'] * 100):.1f}%")
    with col3:
        st.metric("LCOS Advantage", f"${lcos_adv:.2f}", f"{(lcos_adv / overbuild_data['summary']['lcos'] * 100):.1f}%")
    with col4:
        ...
        st.metric("Utilization", f"{util_st:.1f}%", f"+{(util_st - util_ob):.1f}%")
    ...
    with col1:
        ...
        fig.add_trace(go.Bar(x=["Overbuild", "Staged"],
                            ...
                            text=[f"${overbuild_data['summary']['npv']:.2f}M", f"${staged_data['summary']['npv']:.2f}M"],
    ...
    with col2:
        ...
        fig.add_trace(go.Bar(x=["Overbuild", "Staged"],
                            ...
                            marker_color=["#ff6b6b", "#51cf66"],
                            text=[f"${overbuild_data['summary']['lcos']:.2f}", f"${staged_data['summary']['lcos']:.2f}"],
                            textposition="outside"))
    ...
    fig.add_trace(go.Scatter(x=ob_cf["year"], y=ob_cf["capacity"], name="Overbuild", 
                            mode="lines+markers", line=dict(color="red", width=2)))
    fig.add_trace(go.Scatter(x=st_cf["year"], y=st_cf["capacity"], name="Staged", 
...
def generate_csv_reports(overbuild_data, staged_data):
    output = io.StringIO()
    output.write("BESS LCOS ANALYSIS - REPORT\n")
    ...
    output.write(f"NPV ($M),{overbuild_data['summary']['npv']:.2f},{staged_data['summary']['npv']:.2f},{staged_data['summary']['npv']-overbuild_data['summary']['npv']:.2f}\n")
...
def generate_docx_report(overbuild_data, staged_data):
    ...
    timestamp_para = doc.add_paragraph(f"Generated: {datetime.now().strftime('%B %d, %Y')}")
    timestamp_para.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph()
    doc.add_heading('Executive Summary', level=1)
    doc.add_paragraph(f"NPV Advantage: ${staged_data['summary']['npv'] - overbuild_data['summary']['npv']:.2f}M")
    doc.add_paragraph(f"CAPEX Savings: ${overbuild_data['summary']['initial_capex'] - staged_data['summary']['initial_capex']:.2f}M")
    doc.add_heading('Recommendation', level=2)
    doc.add_paragraph("Staged Augmentation approach is financially superior.")
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio("Select Page:", ["Input & Analysis", "Generate Reports"])
...
def show_input_analysis():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["1️⃣ Setup", "2️⃣ SOH", "3️⃣ Augmentation", "4️⃣ Run"])
    
    with tab1:
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Project Parameters")
            power_mw = st.number_input("Power (MW)", value=100, min_value=10, max_value=500, key="power")
            energy_mwh = st.number_input("Energy (MWh)", value=400, min_value=50, max_value=2000, key="energy")
            project_life = st.number_input("Life (years)", value=20, min_value=10, max_value=40, key="life")
            cycles_per_year = st.number_input("Cycles/year", value=365, min_value=100, max_value=1000, key="cycles")
        
        with col2:
            st.subheader("Financial")
            battery_cost = st.number_input("Battery ($/kWh)", value=241, min_value=100, max_value=500, key="cost")
            cost_decline = st.slider("Cost Decline (%/yr)", -8, 2, -4, key="decline") / 100
            inflation_rate = st.slider("Inflation (%/yr)", 0, 5, 2, key="inflation") / 100
            discount_rate = st.slider("Discount (%)", 3, 15, 7, key="discount") / 100
            energy_margin = st.number_input("Margin ($/MWh)", value=50, min_value=10, max_value=200, key="margin")
    
    with tab2:
        st.subheader("SOH Curve")
        st.markdown('<div class="info-box">Enter battery degradation over time</div>', unsafe_allow_html=True)
        
        soh_method = st.radio("Method:", ["Quick (5-yr)", "Detailed"], key="method")
        
        soh_dict = {}
        if soh_method == "Quick (5-yr)":
            cols = st.columns(5)
            with cols[0]: soh_dict[0] = st.number_input("Y0 (%)", 100, 100, 100, key="s0")
            with cols[1]: soh_dict[5] = st.number_input("Y5 (%)", 60, 100, 88, key="s5")
            with cols[2]: soh_dict[10] = st.number_input("Y10 (%)", 60, 100, 75, key="s10")
            with cols[3]: soh_dict[15] = st.number_input("Y15 (%)", 60, 100, 63, key="s15")
            with cols[4]: soh_dict[20] = st.number_input("Y20 (%)", 60, 100, 60, key="s20")
        else:
            cols = st.columns(5)
            for i in range(int(project_life) + 1):
                with cols[i % 5]:
                    default = max(100 - 5 - (i-1)*2.5, 60) if i > 0 else 100
                    soh_dict[i] = st.number_input(f"Y{i}", 50, 100, int(default), key=f"sy{i}")
        
        fig = go.Figure()
        years = sorted(soh_dict.keys())
        soh_vals = [soh_dict[y] for y in years]
        fig.add_trace(go.Scatter(x=years, y=soh_vals, mode='lines+markers', line=dict(color='#1976d2', width=3)))
        fig.update_layout(title="SOH Preview", xaxis_title="Year", yaxis_title="SOH (%)", height=300)
        st.plotly_chart(fig, use_container_width=True, key="soh_preview")
    
    with tab3:
        st.subheader("Augmentation")
        st.markdown('<div class="info-box">Configure capacity additions</div>', unsafe_allow_html=True)
        
        num_aug = st.number_input("Number", 0, 5, 2, key="num")
        ...
        if num_aug > 0:
            ...
            for i in range(num_aug):
                with cols[i]:
                    ...
                    yr = st.number_input("Year", 1, int(project_life)-1, min(7+i*7, int(project_life)-1), key=f"ay{i}")
                    mwh = st.number_input("MWh", 10, 500, 60-i*10, key=f"am{i}")
                    augmentations.append((yr, mwh))
            
            timeline = " → ".join([f"Y{y}:+{m}" for y, m in augmentations])
            st.success(f"📅 {timeline}")
    ...
    with tab4:
        ...
        if st.button("🚀 RUN ANALYSIS", type="primary", key="run"):
            with st.spinner("Calculating..."):
                try:
                    degradation = DegradationProfile(soh_dict)
                    
                    if "Initial" in approach or "Both" in approach:
                        st.session_state.overbuild_data = calculate_overbuild_cashflow(
                    ...
                    if "Augmentation" in approach or "Both" in approach:
                        st.session_state.staged_data = calculate_staged_cashflow(
                            power_mw, energy_mwh, project_life, cycles_per_year,
                            battery_cost, cost_decline, inflation_rate, discount_rate,
    ...
    if st.session_state.analysis_run:
        ...
        if st.session_state.overbuild_data:
            show_analysis_results("Initial Build (Overbuild)", st.session_state.overbuild_data, "overbuild")
        
        if st.session_state.staged_data:
            show_analysis_results("Augmentation (Staged)", st.session_state.staged_data, "staged")
        
        if st.session_state.overbuild_data and st.session_state.staged_data:
            show_comparison_analysis(st.session_state.overbuild_data, st.session_state.staged_data)

def show_reports():
    st.markdown('<div class="section-header">📋 Reports</div>', unsafe_allow_html=True)
    
    if not st.session_state.analysis_run or not st.session_state.overbuild_data or not st.session_state.staged_data:
        st.warning("⚠️ Run analysis with 'Both (Comparison)' first")
        return
    ...
    with col1:
        st.markdown("### CSV")
        csv_data = generate_csv_reports(st.session_state.overbuild_data, st.session_state.staged_data)
        st.download_button("📥 CSV", csv_data, 
                          f"BESS_Analysis_{datetime.now().strftime('%Y%m%d')}.csv", 
                          "text/csv", key="csv")
    
    with col2:
        st.markdown("### DOCX")
        docx_data = generate_docx_report(st.session_state.overbuild_data, st.session_state.staged_data)
    ...
    with col3:
        ...
        summary_df = pd.DataFrame({
            "Metric": ["CAPEX", "NPV", "LCOS"],
            "Overbuild": [f"{st.session_state.overbuild_data['summary']['initial_capex']:.2f}",
                         f"{st.session_state.overbuild_data['summary']['npv']:.2f}",
                         f"{st.session_state.overbuild_data['summary']['lcos']:.2f}"],
            "Staged": [f"{st.session_state.staged_data['summary']['initial_capex']:.2f}",
                      f"{st.session_state.staged_data['summary']['npv']:.2f}",
                      f"{st.session_state.staged_data['summary']['lcos']:.2f}"]
        })
        st.download_button("📥 Summary", summary_df.to_csv(index=False),
