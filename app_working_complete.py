# BESS LCOS Analysis - Complete Working Version 3.0
# Full User Control with SOH, Augmentation, and Report Generation
...
def show_analysis_results(title, data, approach_type):
    ...
    with tab1:
        ...
        with col2:
            ...
            fig.add_trace(go.Scatter(x=df["year"], y=df["cumulative_pv"], mode="lines+markers",
                                    line=dict(color="darkblue", width=3), name="Cumulative NPV"))
            fig.add_hline(y=0, line_dash="dash", line_color="red", annotation_text="Break-even")
    ...
    with tab2:
        ...
        with col2:
            ...
            fig.update_layout(title="Battery Degradation (SOH)", xaxis_title="Year", yaxis_title="SOH (%)",
                            height=400)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        fig.add_trace(go.Scatter(x=df["year"], y=df["capacity"], mode="lines+markers",
                                line=dict(color="orange", width=3), fill="tozeroy", name="Available Capacity"))
    ...
    with tab4:
        ...
        st.download_button(
            ...
            mime="text/csv",
            key=f"download_{approach_type}"
        )

def show_comparison_analysis(overbuild_data, staged_data):
    st.markdown('<div class="section-header">⚖️ Comparative Analysis - Overbuild vs Staged</div>', unsafe_allow_html=True)
    ...
    with col1:
        st.metric("NPV Advantage ($M)", f"${npv_adv:.2f}",
                 delta=f"{(npv_adv / abs(overbuild_data['summary']['npv']) * 100):.1f}% better",
                 delta_color="off")
    with col2:
        st.metric("CAPEX Savings ($M)", f"${capex_adv:.2f}",
                 delta=f"{(capex_adv / overbuild_data['summary']['initial_capex'] * 100):.1f}% reduction",
                 delta_color="off")
    with col3:
        st.metric("LCOS Advantage ($/MWh)", f"${lcos_adv:.2f}",
                 delta=f"{(lcos_adv / overbuild_data['summary']['lcos'] * 100):.1f}% lower",
                 delta_color="off")
    with col4:
        ...
        st.metric("Utilization Rate", f"{util_st:.1f}%",
                 delta=f"+{(util_st - util_ob):.1f}% vs Overbuild",
                 delta_color="off")
    ...
    with col1:
        ...
        fig.update_layout(title="NPV Comparison", yaxis_title="NPV ($ Millions)", height=400)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col2:
        ...
        fig.update_layout(title="LCOS Comparison", yaxis_title="LCOS ($/MWh)", height=400)
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig.add_trace(go.Scatter(
        x=ob_cf["year"], y=ob_cf["capacity"],
        name="Overbuild", mode="lines+markers",
        line=dict(color="red", width=2)
    ))
    fig.add_trace(go.Scatter(
        x=st_cf["year"], y=st_cf["capacity"],
        name="Staged", mode="lines+markers",
        line=dict(color="green", width=2)
    ))
    fig.update_layout(
        title="Available Capacity Comparison - Overbuild vs Staged",
        xaxis_title="Year", yaxis_title="Available Capacity (MWh)",
...
def generate_csv_reports(overbuild_data, staged_data):
    output = io.StringIO()
    output.write("BESS LCOS ANALYSIS - COMPREHENSIVE REPORT\n")
    ...
    output.write("\n" + "="*80 + "\n")
    output.write("OVERBUILD APPROACH - YEAR BY YEAR CASH FLOW\n")
    output.write("="*80 + "\n\n")
    overbuild_data["cashflow"].to_csv(output, index=False)
    output.write("\n\n" + "="*80 + "\n")
    output.write("STAGED AUGMENTATION APPROACH - YEAR BY YEAR CASH FLOW\n")
    output.write("="*80 + "\n\n")
...
def generate_docx_report(overbuild_data, staged_data):
    ...
    doc.add_paragraph(f"NPV Advantage (Staged): ${staged_data['summary']['npv'] - overbuild_data['summary']['npv']:.2f}M")
    doc.add_paragraph(f"CAPEX Savings (Staged): ${overbuild_data['summary']['initial_capex'] - staged_data['summary']['initial_capex']:.2f}M")
    doc.add_paragraph(f"LCOS Advantage (Staged): ${overbuild_data['summary']['lcos'] - staged_data['summary']['lcos']:.2f}/MWh")
    doc.add_heading('Recommendation', level=2)
    doc.add_paragraph("The Staged Augmentation approach is financially superior to the Overbuild approach across all key metrics.")
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio("Select Page:", ["Input & Analysis", "Compare Scenarios", "Generate Reports"])
...
def show_input_analysis():
    ...
    with tab2:
        ...
        st.markdown('<div class="info-box"><b>💡 Instructions:</b> Enter SOH percentage for each year. SOH represents remaining battery capacity (100% = new, 60% = degraded)</div>', unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.markdown('<div class="info-box"><b>💡 Augmentation:</b> Adding battery capacity at specific years instead of building everything upfront</div>', unsafe_allow_html=True)
    ...
    with tab4:
        st.subheader("🎯 Select Analysis Approach & Run")
        
        approach = st.selectbox(
            "Choose approach to analyze:",
    ...
    if st.session_state.analysis_run:
        ...
        if st.session_state.overbuild_data:
            show_analysis_results("Initial Build Approach (Overbuild)", st.session_state.overbuild_data, "overbuild")
        
        if st.session_state.staged_data:
            show_analysis_results("Augmentation Approach (Staged)", st.session_state.staged_data, "staged")
...
def show_scenario_comparison():
    ...
    st.info("💡 Configure settings in 'Input & Analysis' tab first, then run analysis there to see scenario comparisons")
...
def show_reports():
    ...
    if not st.session_state.analysis_run or not st.session_state.overbuild_data or not st.session_state.staged_data:
        st.warning("⚠️ Please run analysis first in 'Input & Analysis' tab with 'Both (Comparison)' selected")
        return
    ...
    with col1:
        ...
        st.info("Complete cash flow data (Excel-ready)")
    ...
    with col2:
        ...
        st.info("Professional Word document")
    ...
    with col3:
        ...
        st.info("Executive summary metrics")
        summary_data = {
            "Metric": ["Initial CAPEX ($M)", "Project NPV ($M)", "LCOS ($/MWh)", "Avg Capacity (MWh)"],
            "Overbuild": [
                f"{st.session_state.overbuild_data['summary']['initial_capex']:.2f}",
                f"{st.session_state.overbuild_data['summary']['npv']:.2f}",
                f"{st.session_state.overbuild_data['summary']['lcos']:.2f}",
                f"{st.session_state.overbuild_data['summary']['avg_capacity']:.1f}"
            ],
            "Staged": [
                f"{st.session_state.staged_data['summary']['initial_capex']:.2f}",
                f"{st.session_state.staged_data['summary']['npv']:.2f}",
                f"{st.session_state.staged_data['summary']['lcos']:.2f}",
                f"{st.session_state.staged_data['summary']['avg_capacity']:.1f}"
            ]
        }
