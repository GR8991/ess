...
# Bi-exponential fault current decay model
def fault_current_biexp(t, I0, A, alpha, beta):
    return I0 * (A * np.exp(-alpha * t) + (1 - A) * np.exp(-beta * t))

# Piecewise interpolation for time-current curves (log-log)
def interpolate_time_current(I_by_Ir, data_points):
    # data_points = [(I/I_r, t_trip), ...] log-log scale data
...
# Example tabulated data points: fuse and breaker (I/I_r, trip_time_s)
...
I_nom = (p_rack_mw * 1e6) / v_dc
I_total = I_nom * n_racks
st.sidebar.markdown(f"**Nominal Current (Container Level):** {I_total:.0f} A")

I0 = st.sidebar.slider("Initial Fault Current I0 (A)", min_value=I_total, max_value=5*I_total, value=3*I_total, step=1000)
...
# Relative current I/I_r for curves
I_by_Ir = fault_current / (I_total * 1.25)  # Assume fuse rating = 1.25 * I_total

fuse_trip = interpolate_time_current(I_by_Ir, fuse_data)
breaker_trip = interpolate_time_current(I_by_Ir, breaker_data)

# Plot fault current and T-I curves
...
if gf_current >= gf_threshold:
    st.error(f"Ground fault detected! Current {gf_current} A exceeds threshold {gf_threshold} A.")
else:
    st.success("No ground fault detected.")
