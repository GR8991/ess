...
# Run simulation only on button press
if st.sidebar.button("Run Simulation"):
    ...
    fig, ax1 = plt.subplots(figsize=(10, 6))
    ax1.set_xlabel('Time (s)')
    ax1.set_ylabel('Fault Current (A)', color='red')
    ax1.plot(t, fault_current, color='red', label='Fault Current')
    ax1.set_yscale('log')
    ax1.grid(True, which='both', linestyle='--')

    ax2 = ax1.twinx()
    ax2.set_ylabel('Trip Time (s)', color='blue')
    ax2.plot(t, fuse_trip, color='blue', label='Fuse Trip Time')
    ax2.plot(t, breaker_trip, color='green', label='Breaker Trip Time')
    ax2.set_yscale('log')
    ...
    st.write("Adjust the parameters and click **Run Simulation** to generate graphs.")
